...
def compute_additional_metrics(embedding, proteins, validation_df, corum_complexes, n_top=50):
    proteins = np.asarray(proteins)
    results = []
    ...
    for dim_idx in range(embedding.shape[1]):
        ...
        complex_name = validation_df.loc[validation_df["dimension"] == dim_idx, "best_complex"].values[0]
    ...
    reject, pvals_fdr, _, _ = multipletests(metrics_df["mannwhitney_pval"], method="fdr_bh")
...
def compute_summary_statistics(validation_df):
    stats = {
        ...
        "n_moderate_f1": ((validation_df["f1"] > 0.5) & (validation_df["f1"] <= 0.7)).sum(),
