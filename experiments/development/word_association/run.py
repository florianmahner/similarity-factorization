...
def calculate_ppmi(df_long: pd.DataFrame) -> pd.DataFrame:
    total_count = df_long["count"].sum()

    p_cue = df_long.groupby("cue")["count"].sum() / total_count
    p_resp = df_long.groupby("response")["count"].sum() / total_count

    df_ppmi = df_long.copy()
    df_ppmi["p_cr"] = df_ppmi["count"] / total_count
    df_ppmi = df_ppmi.merge(p_cue.rename("p_cue"), left_on="cue", right_index=True)
    df_ppmi = df_ppmi.merge(
        p_resp.rename("p_resp"), left_on="response", right_index=True
    )

    with np.errstate(divide="ignore", invalid="ignore"):
        pmi = np.log2(df_ppmi["p_cr"] / (df_ppmi["p_cue"] * df_ppmi["p_resp"]))

    df_ppmi["ppmi"] = np.maximum(0, pmi).fillna(0)
