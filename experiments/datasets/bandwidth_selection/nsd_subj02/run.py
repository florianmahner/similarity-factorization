"""Bandwidth (sigma_scale alpha) selection for NSD subj02."""
...
if __name__ == "__main__":
    t0 = time.time()
    print(f"[{time.strftime('%H:%M:%S')}] loading NSD subj02 features...", flush=True)
    ds = load_dataset(
        "nsd",
        root="/data/labshare/_stachelschwein/LOCAL/LABSHARE/natural-scenes-dataset",
        subject_id=2, roi_name="streams", space="func1pt8mm",
    ...
    run_bandwidth_selection(
        dataset_name="nsd_subj02", features=features,
