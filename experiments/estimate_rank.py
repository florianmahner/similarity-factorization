...
def run(cfg: DictConfig) -> None:
    ...
    ax.errorbar(ranks, means, yerr=stds, fmt="o-", color=TEAL, capsize=3, markersize=4)
    ax.axvline(optimal_rank, color=ROSE, linestyle="--", linewidth=1.5, label=f"Optimal: {optimal_rank}")
    ax.set_xlabel("Rank")
