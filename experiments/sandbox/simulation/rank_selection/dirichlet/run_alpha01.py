"""CV curves for alpha=0.1 across different k values.

Usage:
    poetry run python sandbox/rank_selection/dirichlet/run_alpha01.py
"""
...
def main():
    ...
    for i, k in enumerate(true_ranks):
        ...
        print(f"  k={k}: p_mean={p_mean:.3f}, gap={gap:.4f}")
    ...
    fig.suptitle(f'CV curves: n={n}, alpha={alpha} (15 repeats)', fontsize=11, y=1.02)
    plt.tight_layout()
    save_figure(fig, OUTPUT_DIR / f'cv_curves_alpha{alpha}_n{n}.pdf')
    print(f'\nSaved: cv_curves_alpha{alpha}_n{n}.pdf')
    print(f'Output: {OUTPUT_DIR}')
