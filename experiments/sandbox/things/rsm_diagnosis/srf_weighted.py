...
try:
    from pysrf._bsum import update_w_blas_blocked as _update_w_impl
...
def update_v_(
    ...
    Args:
        ...
        weights: Continuous weights per entry (None = binary mask)
    """
    v[:] = lam
    v /= -rho
    v += x_hat
    ...
    if weights is not None:
        # Weighted V-update: V = (w*S + rho*WW^T - lam) / (w + rho)
        v[observed_mask] = (
            weights[observed_mask] * x[observed_mask]
            + rho * x_hat[observed_mask]
            - lam[observed_mask]
        ) / (weights[observed_mask] + rho)
    else:
        v[observed_mask] = (
...
class SRF(TransformerMixin, BaseEstimator):
    ...
    def fit(self, x: np.ndarray, y: np.ndarray | None = None) -> SRF:
        ...
        # Store weights (None = unweighted)
        self._weights = getattr(self, '_input_weights', None)

        if self._weights is not None:
            # Weighted mode always uses ADMM path
            return self._fit_missing_data(x)
        elif np.all(self._observation_mask):
