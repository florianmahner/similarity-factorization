...
def evaluate_binary_classification(
    ...
    vocabulary: list[str],
    ratings: pd.DataFrame,
    percentile: int = 20,  # Top/Bottom 20%
) -> pd.DataFrame:
    """
    Checks if the embeddings can linearly separate the High vs Low poles.
    This is often cleaner than regression for noisy behavioral data.
    """
    word_to_idx = {w.lower(): i for i, w in enumerate(vocabulary)}
    dimensions = [col for col in ratings.columns if col != "word"]
    results = []
    ...
    for dimension in dimensions:
        ...
        # Create Binary Classes (High vs Low)
        low_thresh = np.percentile(valid[dimension], percentile)
        high_thresh = np.percentile(valid[dimension], 100 - percentile)

        mask_low = valid[dimension] <= low_thresh
        mask_high = valid[dimension] >= high_thresh

        # Combine
        binary_data = valid[mask_low | mask_high].copy()
        binary_data["label"] = (binary_data[dimension] >= high_thresh).astype(int)

        if len(binary_data) < 50:
            continue

        word_indices = np.array([word_to_idx[w] for w in binary_data["word"]])
        X = embeddings[word_indices]
        y = binary_data["label"].to_numpy()
        ...
        for train_idx, test_idx in kf.split(X, y):
            ...
            clf = LogisticRegressionCV(
                cv=5, penalty="l1", solver="liblinear", random_state=42
            )
            clf.fit(X_train_s, y_train)
            accuracies.append(clf.score(X_test_s, y_test))

        results.append(
            {
                "dimension": dimension,
                "method": "Logistic_L1_Binary",
