...
def plot_power_paper(
    ...
    Style: blue SRF, red RSA, smooth lines with SEM shaded bands.
    """
    grouped = df.groupby(["snr", "method"])["significant"].agg(["mean", "sem"]).reset_index()
    ...
    for method, color, label in styles:
        m = grouped[grouped["method"] == method].sort_values("snr")
        if m.empty:
            continue
        snr = m["snr"].values
        power = m["mean"].values * 100
        ci = m["sem"].values * 100 * 1.96  # 95% CI

        ax.plot(snr, power, color=color, lw=1.5, label=label, zorder=2)
        ax.fill_between(snr, power - ci, power + ci, color=color, alpha=0.2, zorder=1)
