...
Evaluates imputation methods (Mean, Median, KNN, SRF) on synthetic similarity
matrices with missing values at various retention rates.

Usage:
    poetry run python experiments/analyses/simulation/imputation/run.py
"""
...
from src.tools.metrics import compute_similarity
...
N_DATASETS = 10
N_REPLICATES = 5
FRACTION_RETAINED = np.geomspace(0.034, 0.75, 35).tolist()
...
def main() -> None:
    ...
    log.info(f"Running imputation analysis: n={N}, k={K}")
    rng = np.random.default_rng(SEED)

    log.info(f"Generating {N_DATASETS} datasets...")
    dataset_seeds = rng.integers(0, 1_000_000, size=N_DATASETS)
    datasets = Parallel(n_jobs=-1)(
        delayed(_generate_dataset)(N, K, KERNEL, int(seed))
    ...
    for dataset_idx, similarity in enumerate(datasets):
        for fraction_retained in FRACTION_RETAINED:
            for replicate in range(N_REPLICATES):
                tasks.append((similarity, dataset_idx, fraction_retained, replicate, K, mask_seed))
