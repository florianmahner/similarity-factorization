...
# Parameters are now loaded from config.toml
        ...
        for v in sorted(data.keys()):
            if v.startswith('human_'):
                ...
                if g is not None:
                    group_labels[v] = g
    if group_labels:
        return family_name, fit_cca(data, group_labels=group_labels, demean_groups=True, **params)
    return family_name, fit_cca(data, **params)
