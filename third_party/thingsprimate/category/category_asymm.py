...
def plot_scatter(df, out_path, thresholds, sig_col):
    ...
    ax.text(df.loc[most_human_idx, 'monkey_rel_z'] + 0.1,
            df.loc[most_human_idx, 'human_rel_z'] + 0.1,
            df.loc[most_human_idx, 'category'], fontsize=9)
    ax.text(df.loc[most_monkey_idx, 'monkey_rel_z'] + 0.1,
            df.loc[most_monkey_idx, 'human_rel_z'] + 0.1,
            df.loc[most_monkey_idx, 'category'], fontsize=9)
...
def plot_asymmetry_bars(df, out_path, top_k=12):
    ...
    # Set up x-axis with fixed limits as requested
    ax.set_xlim(-7, 7)
    ax.set_ylim(-0.5, top_k - 0.5)  # Tight to bars

    # Add x-axis label as requested
