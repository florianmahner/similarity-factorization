...
# Config ----------------------------------------------------------------------
FEATURES = ['symbol', 'icon', 'numerosity', 'face', 'body_part', 'random']
RESULTS_DIR = Path(config.root_dir) / 'symbol' / 'results'
FIG_DIR = config.fig_dir / 'symbol' / 'decoding'
FIG_SUPP_DIR = config.fig_dir / 'symbol' / 'supplementary'
LEGEND_DIR = FIG_DIR / 'legend'
STATS_DIR = Path(config.root_dir) / 'symbol' / 'results' / 'statistics'
...
def get_feature_colors(region=None):
    ...
    numerosity_color = darken_color(base_icon, mix=NUMEROSITY_DARKEN_MIX)
