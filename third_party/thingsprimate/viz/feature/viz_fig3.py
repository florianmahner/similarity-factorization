...
OUT_DIR = config.fig_dir / 'feature' / 'fig3'
...
def load_features(all_stims, pc1_scores):
    """Load animacy (lives), curvature, and PC1 (as spiky proxy) aligned to CCA stimuli."""
    ...
    # Map curvature - already processed (in [0, 1])
    ...
    print(f'  PC1 (spiky proxy): {len(pc1_norm)} valid, range [{np.min(pc1_norm):.3f}, {np.max(pc1_norm):.3f}]')

    return {'animacy': lives_norm, 'curvature': curv, 'spiky': pc1_norm}

def plot_scatter(comps, feat_vals, feat_name, cmap_name, c1=0, c2=1, highlight_top_n=None, highlight_idxs=None):
    ...
    out = OUT_DIR / f'scatter_{feat_name}.{FMT}'
...
def plot_nonliving_negative_comp1_grid(comps, stims, cats, lives_vals, n=40, n_cols=8):
    ...
    out = OUT_DIR / f'nonliving_negative_comp1_grid.{FMT}'
    fig.savefig(out, bbox_inches='tight', dpi=600)
    plt.close(fig)
    print(f'Saved non-living negative Component 1 grid: {out}')

def plot_variances_with_stats(results):
    ...
    out = OUT_DIR / f'fig3_alexnet_fc6_pc1{suffix}.{FMT}'
    fig.savefig(out, bbox_inches='tight', dpi=600)
    plt.close(fig)
    print(f'\nSaved figure: {out}')
    ...
    suffix = f'_all{n_comp}' if TOP_N is None else f'_top{TOP_N}'
    stats_out = OUT_DIR / f'fig3_alexnet_fc6_pc1{suffix}_stats.csv'
...
def main():
    ...
    print(f'\n=== Fig3: AlexNet FC6 PC1 with {n_str} cross-species CCA components (CV ridge) ===')
    X = load_alexnet_fc6()
    ...
    plot_scatter(comps_all, features['spiky'], 'spiky', 'cividis', c1=0, c2=1)
