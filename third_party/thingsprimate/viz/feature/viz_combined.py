...
OUT_DIR = config.fig_dir / 'feature'
FMT = config.plotting.get('savefig_format', 'pdf')
GROUPS = ['macaque', 'human']
LEGEND_DIR = config.fig_dir / 'feature' / 'legend'
...
def metric_stats(entry: dict) -> tuple[float, float]:
    if SET_METRIC == 'r':
        if entry.get('r_folds'):
            v = np.array(entry['r_folds'], float)
            return float(v.mean()), float(v.std())
        # fallback from r2 if needed (legacy)
        mu2 = float(entry.get('r2_mean', np.nan))
        sd2 = float(entry.get('r2_std', np.nan))
        mu  = math.sqrt(max(mu2, 0.0))
        sd  = (sd2 / (2.0 * max(mu, 1e-6))) if np.isfinite(sd2) else np.nan
        return mu, sd
    else:  # 'r2'
        if entry.get('r2_folds'):
            v = np.array(entry['r2_folds'], float)
            return float(v.mean()), float(v.std())
        return float(entry.get('r2_mean', np.nan)), float(entry.get('r2_std', np.nan))
...
def _score_stats(entry: dict) -> tuple[float, float]:
    if 'r2_folds' in entry and entry['r2_folds']:
        r2f = np.array(entry['r2_folds'], float)
        return float(r2f.mean()), float(r2f.std())
    mu = float(entry.get('r2_mean', np.nan))
    sd = float(entry.get('r2_std', np.nan))
    return mu, sd
...
def load_csv_results(csv_path: Path):
    ...
    for _, row in df.iterrows():
        ...
        # Check for R columns first, fallback to R²
        r_folds = []
        for i in range(1, 11):
            if f'r_f{i}' in row and pd.notna(row[f'r_f{i}']):
                r_folds.append(float(row[f'r_f{i}']))
        if not r_folds:  # Fallback to R²
            for i in range(1, 11):
                if f'r2_f{i}' in row and pd.notna(row[f'r2_f{i}']):
                    r_folds.append(math.sqrt(max(float(row[f'r2_f{i}']), 0.0)))

        entry = {
            'r_mean': float(row.get('r_mean', math.sqrt(max(float(row.get('r2_mean', 0.0)), 0.0)))),
            'r_std': float(row.get('r_std', float(row.get('r2_std', 0.0)) / (2.0 * math.sqrt(max(float(row.get('r2_mean', 1e-12)), 1e-12))))),
...
def load_other_results():
    results = {}
    csv_path = config.results_dir / 'feature' / 'species' / 'cca' / 'other_features.csv'
...
def load_combined_results():
    dnn_res = load_csv_results(config.results_dir / 'feature' / 'species' / 'cca' / 'dnn_features.csv')
    ...
    def plot_group(grp: str):
        ...
        for m in models:
            ...
            if depth.size > 0:
                ...
                d = (depth - dmin) / (dmax - dmin + 1e-9) * 100.0
            else:
                d = depth
