...
# ──────────────────────────────────────────────────────────────────────
# Project imports (ensure repository root on path)
...
from functions.viz_common import plot_duplets_from_enc_scores, make_legend, load_enc_scores, load_enc_matrix
...
# Sandbox figure root (keep all outputs local to this folder, under repository sandbox/)
SANDBOX_FIG_DIR = os.path.join(module_path, 'sandbox', 'figs')
...
# New constants for overview augmentation
...
# Selected components (1-indexed), defaults to 1..4
CCA_SELECTED = { 'cross-species': [1,2,3,4,5,6,7,8], 'human': [1,2,3,4], 'monkey': [1,2,3,4] }
...
def subsample_top_pct(vec, n_cells, pct=TOP_PCT, pos=True):
    ...
    return sorted_idx[:min(n_cells, len(sorted_idx))]
...
def plot_overview_cca_with_duplet_bars(comps, all_stims, cats, img_dir, save_path, family_name,
    ...
    Bars now have no rotated x tick labels; feature names shown in two-column text table (Visual | Behav), bold, no numbers.
    """
...
def save_fig(fig, fig_type, analysis_family, comp_idx=None, extra_info="", show_inline=False):
    ...
    # Save inside sandbox output tree, mirroring the original structure
    base = os.path.join(SANDBOX_FIG_DIR, f"cca_analysis/{analysis_family}/{fig_type}")
    os.makedirs(base, exist_ok=True)
    fmt = config.plotting.get('savefig_format', 'pdf')
    if fig_type == 'duplets' and comp_idx is None and not extra_info:
        fname = 'bar_duplet.pdf'; fmt = 'pdf'
    ...
    dpi = 600 if image_high_dpi or fig_type in ('images', 'selected') else 300
...
if __name__ == "__main__":
    ...
    os.makedirs(SANDBOX_FIG_DIR, exist_ok=True)
    
    # Clear CCA analysis folder before writing new figures
    cca_fig_dir = os.path.join(SANDBOX_FIG_DIR, 'cca_analysis')
    ...
    for fam in TARGET_FAMILIES:
        ...
        overview_path = os.path.join(SANDBOX_FIG_DIR, 'cca_analysis', analysis_family_name, 'overview', 'components_overview.png')
        ...
        if imp_mean is not None and feature_names is not None and feature_groups is not None:
            ...
            # Save CSV summary
            top_dir = os.path.join(SANDBOX_FIG_DIR, 'cca_analysis', analysis_family_name, 'top_features')
