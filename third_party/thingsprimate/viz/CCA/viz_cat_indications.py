...
# Ensure project modules are on path (match repo convention)
...
def plot_top_imgs(avg_load, stims, cats, title, n_top=8, img_sz=60, zm=0.55):
    ...
    for bar, i, stim in zip(bars, top_idx, top_stims):
        ...
        try:
            im = Image.open(path).convert('RGB').resize((img_sz, img_sz), Image.BILINEAR)
            arr = np.array(im)
...
if __name__=="__main__":
    ...
    # Load face/body maps from annotations
    annot_fp = Path(config.data_dir) / 'things' / 'annotations.csv'
