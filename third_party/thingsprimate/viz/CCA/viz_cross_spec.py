...
col_grey = config.plotting.get("nonsignificant_color", "#B0B0B0")
...
def _add_gradient_like_scree(ax, lo, hi, n_bands=50):
    ...
    for i, y0 in enumerate(y_fill[:-1]):
        y1 = y_fill[i + 1]
        # Match the alpha schedule from the provided scree code (darker at lower y)
        alpha_val = 0.2 * (1 - (y0 - lo) / denom)
        ax.fill_between(x_line, y0, y1, alpha=alpha_val, color="gray",
                        edgecolor="none", zorder=0)

def _species_generalization_map(family_stats, family_colors, title, out_path):
    """
    ...
    for fam, st in family_stats.items():
        ...
        # Non-significant points (grey), WITH dark grey outline
        if x_nsg.size:
            ax.scatter(
                x_nsg, y_nsg, s=38, alpha=0.85,
                edgecolors=edge_col, linewidths=0.6,
                c=col_grey, label=None, zorder=2
            )
    ...
    # Limits & labels (+ non-italic R^2 via \mathrm)
    ax.set_xlim(lo, hi); ax.set_ylim(lo, hi)
    y_ticks = ax.get_yticks()
    if y_ticks.size:
        xticks = [t for t in y_ticks if lo <= t <= hi]
        if len(xticks) >= 2:
            ax.set_xticks(xticks)
    ax.set_xlabel(r"Monkey $\mathrm{R}^2$")
    ax.set_ylabel(r"Human $\mathrm{R}^2$")
    ax.set_title(title)
...
LEG_OUT = OUT / "species_ridge_legend.pdf"
