...
def estimate_p_bound(
    ...
    if eff_dim is not None:
        assert isinstance(eff_dim, int) and 0 < eff_dim <= S.shape[0]
    eff_dim = np.ceil((np.linalg.norm(S, 'fro') / np.linalg.norm(S, 2))**2).astype(int)

    lam_ = np.linalg.eigvalsh(S)[::-1][:int(np.sqrt(S.shape[0]))]
    avg_gap = np.mean( lam_[:-1] - lam_[1:] )
    print( lam_[:-1] - lam_[1:] )
    
    print('estimate eta for pmax')
    print(eta_pmax, np.sqrt(avg_gap) )
    pmax = p_upper_only_k(
        S, 
        k=eff_dim, 
        method=method,
...
def monte_carlo_bulk_edge_raw(
    ...
    iterator = tqdm(
        range(n_trials),
        desc=progress_label or f"mc bulk edge p={float(p):.3f}",
...
def p_upper_only_k_fast(
    ...
    """
    Estimate the largest mask ratio pmax where exactly k spikes remain out.

    This mirrors ``p_upper_only_k`` but uses the direct eigensolver-based bulk-edge
    estimator by default, which is typically faster on smaller matrices. The bulk
    edge is estimated from masked-noise samples using ``eigvalsh`` through
    ``monte_carlo_bulk_edge_raw``.

    If ``method == 'dyson'``, the previous Dyson fast path remains available.
    """
    import numpy as np
    from numpy.linalg import eigvalsh
    ...
    total_steps = 6
    if verbose:
        print(f"[step 1/{total_steps}] preparing eigenvalue computation")

    eval_k = min(n, max(int(k_max), k + 1))
    if verbose:
        mode = 'partial eigsh' if eval_k < n else 'full eigvalsh'
        print(f"  computing top {eval_k} eigenvalues out of n={n} using {mode}")
    if eval_k < n:
        lam = np.sort(
            eigsh(S, k=eval_k, which='LA', return_eigenvectors=False)
        )[::-1]
    else:
        lam = np.sort(eigvalsh(S))[::-1]
    ...
    if lam_k <= 0:
        if verbose:
            print("λ_k ≤ 0 → no positive spike to separate.")
        return 0.0
    if (lam_k1 is None) or (lam_k1 <= 0):
        if verbose:
            print("λ_{k+1} ≤ 0 → only first k can be out for all large p; return 1.0.")
        return 1.0

    edge_cache = {}

    if method == 'dyson':
        from _compute_bulkedge_Dyson import VDEPrecomp, lambda_bulk_dyson_auto_fast

        if verbose:
            print(f"[step 2/{total_steps}] building Dyson precomputation")
        precomp = VDEPrecomp(
            S,
        ...
        def edge(p):
            key = float(np.round(float(p), 15))
            if key not in edge_cache:
                if verbose:
                    print(f"  [edge] computing Dyson bulk edge for p={float(p):.6f}")
                edge_val, info = lambda_bulk_dyson_auto_fast(
                    precomp,
                    float(p),
                    omega=omega,
                    solver=solver,
                ...
                edge_cache[key] = (float(edge_val), info)
            return edge_cache[key][0]
    else:
        if verbose:
            print(f"[step 2/{total_steps}] using direct eigsolve bulk-edge estimator")

        def edge(p):
            key = float(np.round(float(p), 15))
            if key not in edge_cache:
                if verbose:
                    print(f"  [edge] computing eigsolve bulk edge for p={float(p):.6f}")
                edge_cache[key] = (
                    float(
                        monte_carlo_bulk_edge_raw(
                            S,
                            ...
                            progress_label=f"mc bulk edge p={float(p):.3f}",
                        )
                    ),
                    None,
                )
    ...
    if verbose:
        print(f"[step 3/{total_steps}] sanity check at p=0.99")
    c_hi, e_hi = count_out(0.99)
    if verbose:
        print(
            f"[sanity] p=0.99: bulk={e_hi:.4g}, count_out={c_hi}, "
            f"λ1={lam[0]:.4g}, λ2={lam[1] if len(lam) > 1 else np.nan:.4g}"
        )

        if method == 'dyson':
            info_hi = edge_cache[float(np.round(0.99, 15))][1]
            if info_hi is not None:
                print(
                    "[dyson-fast] "
                    f"eta={info_hi['eta']:.3g}, ngrid={info_hi['ngrid']}, "
                    f"jump_frac={info_hi['jump_frac']:.3g}, solver={info_hi['solver']}"
                )
        else:
            print(
                "[eigsolve-fast] "
                f"mc_trials={mc_trials}, quantile={mc_quantile:.3g}"
            )
    ...
    if verbose:
        print(f"[step 4/{total_steps}] coarse grid search over p values")
    grid = np.linspace(0.02, 0.99, 80)
    feas = []
    grid_iter = tqdm(
    ...
    for i, p in grid_iter:
        c_val, _ = count_out(p)
        if verbose and (i == 1 or i == len(grid) or i % 10 == 0):
            print(f"  [grid {i}/{len(grid)}] p={p:.4f}, count_out={c_val}")
        if c_val == k:
            feas.append(p)

    if not feas:
        if verbose:
            print(f"[step 5/{total_steps}] no feasible grid point found; bisecting g(p)")
        ...
        if ga >= 0 and gb >= 0:
            if verbose:
                print(
                    "(k+1) spike is out for all p; returning smallest p "
                    "where count==k (none found) → 0."
                )
            return 0.0
        ...
        for it in bisect_iter:
            ...
            if verbose and (it == 0 or (it + 1) % 10 == 0):
                print(f"  [bisection {it + 1}/60] lo={lo:.6f}, hi={hi:.6f}, g(mid)={g_mid:.6g}")
            if (hi - lo) < tol:
                break

        p_star = max(0.0, min(1.0, lo - 2 * tol))
        if verbose:
            print(f"[step 6/{total_steps}] finished fallback bisection with p*={p_star:.6f}")
        return p_star

    if verbose:
        print(f"[step 5/{total_steps}] refining upper boundary from feasible region")
    p_lo = max(feas)
    ...
    while (p_hi < 0.99) and (not cond_ge_kplus1(p_hi)):
        expand_step += 1
        if verbose:
            print(f"  [expand {expand_step}] p_hi={p_hi:.4f} still has only k spikes out")
        p_hi = min(0.99, p_hi + 0.05)

    if not cond_ge_kplus1(p_hi):
        if verbose:
            print("No p with k+1 outliers found after expansion; returning 1.0.")
        return 1.0
    ...
    bisect_iter = tqdm(
        range(60),
        total=60,
        desc="bisect count_out",
        disable=not verbose,
        leave=False,
    )
    for it in bisect_iter:
        mid = 0.5 * (lo + hi)
        has_kplus1 = cond_ge_kplus1(mid)
        if has_kplus1:
            hi = mid
        else:
            lo = mid
        if verbose and (it == 0 or (it + 1) % 10 == 0):
            print(f"  [boundary {it + 1}/60] lo={lo:.6f}, hi={hi:.6f}, ge_kplus1={has_kplus1}")
        if (hi - lo) < tol:
            break

    p_star = max(0.0, min(1.0, lo))
    if verbose:
        print(f"[step 6/{total_steps}] finalizing result")
        c_star, e_star = count_out(p_star)
        print(f"p*={p_star:.4f}, bulk={e_star:.6g}, count_out(p*)={c_star}")
    return p_star
