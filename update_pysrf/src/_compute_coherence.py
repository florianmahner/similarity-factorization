...
def _randomized_topk_eigenspace_symmetric(
    ...
    Q, _ = la.qr(Y, mode="reduced")  # n x l
    B = Q.T @ A @ Q                 # l x l (small)
    B = 0.5 * (B + B.T)
...
def _fast_iproj_worker_one_boot(args):
    ...
    Also computes the Rayleigh-trace numerator tr(P_k(p) S_ref) used by Recipe K
    (corrected). Backwards-compatible: callers that pass the legacy 14-tuple
    signature (no evals_ref) get trPkS_boot_b = None.
    """
    if len(args) == 15:
        (
            b,
    ...
    trPkS_boot_b = np.zeros((K_count, P), float) if evals_ref is not None else None
    ...
    if show_inner_progress:
        p_iter = enumerate(tqdm(p_list, total=P, desc=f"fast p-sweep b={b + 1}", leave=False))
    ...
    for j, p in p_iter:
        ...
        if trPkS_boot_b is not None:
            # Exact Rayleigh trace numerator tr(P_k(p) S) computed against the
            # full reference matrix S0 (NaN-zeroed) — not against the rank-Kmax
            # truncation. Previous implementation summed lambda_a^ref * G[j',a]^2
            # over a<=Kmax only, which is tr(P_k S_ref^{(Kmax)}), and dropped
            # bulk contributions a>Kmax that the masked top-k subspace can pick
            # up. See RECIPE_K_THESIS_REBUTTAL_ROUND1.md (issue #22, 2026-05-11).
            SUk = S0 @ U_k                                # n × Kmax
            diag_uSu = np.einsum("ij,ij->j", U_k, SUk)    # length Kmax
            trace_cumsum_exact = np.cumsum(diag_uSu)
            trPkS_boot_b[:, j] = trace_cumsum_exact[k_idx]
...
def _eig_worker_one_p(args):
    ...
    # Exact Rayleigh-trace numerator (since 2026-05-11):
    #   trPkS_boot_p[ki, b] = tr(P_k(p) S) = sum_{j' <= k} <U_k[:,j'], S U_k[:,j']>,
    # computed via one S0 @ U_k matvec per bootstrap. Earlier releases stored
    # the rank-Kmax truncation sum_{a<=Kmax} lambda_a^ref * G[j',a]^2 which
    # drops bulk contributions a>Kmax. Used by Recipe K (corrected) for the
    # proper VE^D = tr(P_k S)/tr(S) inversion.
    trPkS_boot_p = np.zeros((K_count, B), float)
    ...
    for b in range(B):
        rng = np.random.default_rng((random_seed_base + 1000003 * i + 9176 * b) & 0xFFFFFFFF)

        A = _masked_unbiased_spd_missing(S0, W, q, p, rng, keep_diag=keep_diag, iu=iu)

        try:
            vals_k, U_k = _topk_eigenvectors(A, Kmax, eigsh=eigsh, tol=eigsh_tol, maxiter=eigsh_maxiter)
        except Exception:
            stats["fail_count"] += 1
        ...
        # Exact Rayleigh-trace numerator tr(P_k(p) S) for Recipe K.
        # Earlier versions stored sum_{a<=Kmax} lambda_a^ref * G[j',a]^2 which
        # is tr(P_k S_ref^{(Kmax)}) — the trace against the rank-Kmax reference
        # reconstruction, dropping bulk contributions a>Kmax. The exact
        # numerator is tr(U_k^T S U_k), computed via one S0 @ U_k matvec per
        # (p, b). See RECIPE_K_THESIS_REBUTTAL_ROUND1.md (issue #22, 2026-05-11).
        SUk = S0 @ U_k                                # n × Kmax
        diag_uSu = np.einsum("ij,ij->j", U_k, SUk)    # length Kmax
        trace_cumsum_exact = np.cumsum(diag_uSu)
        ...
        trPkS_boot_p[:, b] = trace_cumsum_exact[k_idx]
    ...
    return i, p, Iproj_boot_p, C_boot_p, I_boot_p, lambda_topk_boot_p, worker_stats, null_boot_p, trPkS_boot_p
...
def compute_incremental_coherence_multi_k_eig_anisotropic(
    ...
    trPkS_boot = np.zeros((K_count, P, B), float)   # Rayleigh-trace numerator (Recipe K)
    ...
    if n_jobs_eff == 1:
        ...
        pbar = tqdm(total=P, desc=f"eig-based parallel p (n_jobs={n_jobs_eff})", leave=True)
    ...
    for (i, p, Iproj_p, C_p, I_p, lam_topk_p, wstats, null_p, trPkS_p) in results:
        Iproj_boot[:, i, :] = Iproj_p
        C_boot[:, i, :] = C_p
        I_boot[:, i, :] = I_p
        trPkS_boot[:, i, :] = trPkS_p
    ...
    if use_baseline_correction:
        x_boot = _baseline_correct_array(Iproj_boot, k_list, n, clip=clip_baseline)  # (K,P,B)
    else:
    ...
    if null_boot is not None:
        if use_baseline_correction:
            null_x = _baseline_correct_array(null_boot, k_list, n, clip=clip_baseline)  # (K,P,B,B_null)
        else:
    ...
    if visualize and visualize_mode == "plotly":
        ...
        buttons.append(dict(
            label="Means only",
            method="update",
            args=[{"visible": _vis_means_only()},
                  {"title": "Iproj trends (means only)"}],
        ))
        buttons.append(dict(
            label="Means + CI",
            method="update",
            args=[{"visible": _vis_means_plus_ci()},
        ...
        for kk, k in enumerate(k_list):
            buttons.append(dict(
                label=f"Focus k={k}",
                method="update",
                args=[{"visible": _vis_focus_k(kk, show_ci=True, show_tau=True, show_act=True)},
                      {"title": f"Iproj: focus view (k={k})"}],
            ))

        ylab = "Iproj_k(p)" if not use_baseline_correction else "x_k(p) (baseline-corrected)"
        ...
        fig.update_layout(
            ...
            height=700,
            title="Iproj trends (means only)",
            xaxis=dict(title=dict(text="p (masking probability)", standoff=18), automargin=True),
    ...
    return {
        ...
        "trPkS_boot": trPkS_boot,   # Rayleigh-trace numerator (Recipe K corrected)
        "summary": summary,
        "fig": fig,
        "evals_ref": evals_ref,
        "U_ref_K": U_ref_K,
...
def test_fast_coherence(
    ...
    Iproj_boot = np.zeros((K_count, P, B), float)
    trPkS_boot = np.zeros((K_count, P, B), float)
    ...
    if n_jobs_eff == 1:
        if show_progress:
            results = []
            for t in tqdm(tasks, total=B, desc="fast exact Iproj bootstrap", leave=True):
                tt = list(t)
                tt[-2] = True  # show_inner_progress (now second-to-last; -1 is evals_ref)
                results.append(_fast_iproj_worker_one_boot(tuple(tt)))
        else:
            results = [_fast_iproj_worker_one_boot(t) for t in tasks]
    elif show_progress:
        pbar = tqdm(total=B, desc=f"fast exact Iproj bootstrap (n_jobs={n_jobs_eff})", leave=True)
    ...
    for b, Iproj_b, stats_b, trPkS_b in results:
        Iproj_boot[:, :, b] = Iproj_b
        if trPkS_b is not None:
            trPkS_boot[:, :, b] = trPkS_b
    ...
    if visualize and visualize_mode == "plotly":
        ...
        buttons = [
            dict(
                label="Means only",
                method="update",
                args=[{"visible": _vis_means_only()}, {"title": "Fast exact Iproj trends (means only)"}],
            ),
            dict(
                label="Means + CI",
                method="update",
                args=[{"visible": _vis_means_plus_ci()}, {"title": f"Fast exact Iproj trends with {int(ci_level*100)}% CI"}],
            ),
        ]
...
def _mutual_info_matrix_pairwise(X, bins="fd", normalize="sqrt", eps=1e-12):
    ...
    for i in range(K):
        for j in range(i + 1, K):
            val = _mutual_info_discrete(X[i], X[j], bins=bins, normalize=normalize, eps=eps)
            M[i, j] = val
            M[j, i] = val
    return M
...
def _get_pair_metric(metric, mi_bins="fd", mi_normalize="sqrt", eps=1e-12):
    ...
    if m in ("mi", "mutual_info", "mutualinformation", "mutual-information"):
        fn = lambda a, b: _mutual_info_discrete(
            a, b, bins=mi_bins, normalize=mi_normalize, eps=eps
        )
        return fn, (0.0, 1.0) if mi_normalize is not None else (None, None)
    raise ValueError("metric must be 'pearson', 'spearman', 'mi', or a callable(a,b)->float")
...
def analyze_cluster_consensus_across_p(
    ...
    if make_plots:
        try:
            ...
            raise ImportError("plotly is required when make_plots=True. pip install plotly") from e
    ...
    def _similarity_matrix_rows(X_kp):
        ...
        if metric_name in ("mi", "mutual_info", "mutualinformation", "mutual-information"):
            return _mutual_info_matrix_pairwise(X_kp, bins=mi_bins, normalize=mi_normalize, eps=eps)
    ...
    def _cluster_from_similarity(S):
        ...
        elif metric_name in ("pearson", "spearman", "corr", "correlation", "rank", "rankcorr"):
            adj_base = np.abs(S) if use_abs_for_corr else S
        else:
            adj_base = S
    ...
    for ci in range(C):
        ...
        for j in range(0, P - L + 1):
            if np.all(ok[j : j + L]):
                emergence_idx[ci] = j
                break
    emergence_p = np.array([p_list[j] if j >= 0 else np.nan for j in emergence_idx], dtype=float)
...
def plot_consensus_from_p_slider(
    ...
    if make_plot:
        try:
            ...
            raise ImportError("plotly is required when make_plot=True. pip install plotly") from e
...
def _find_first_p_ge_threshold(p_list: np.ndarray, curve: np.ndarray, thr: float) -> Optional[float]:
...
def kappa_changepoint(
    ...
    *,
    ...
    if changepoint_mode == "largest_jump":
        ...
        info = dict(mode="largest_jump", jump_metric=jump_metric, smoothed=sm, diffs=d, score=score, i_star=i_star, k_cut=k_cut)
        return k_cut, info
        ...
        if pos.size == 0:
            i_star = int(valid[np.argmax(score[valid])])
            k_cut = int(k_list[i_star])
            return k_cut, dict(mode="first_above_quantile", reason="no positive score; used argmax", smoothed=sm, diffs=d, score=score, i_star=i_star, k_cut=k_cut)
        ...
        if cand.size == 0:
            i_star = int(valid[np.argmax(score[valid])])
            k_cut = int(k_list[i_star])
            return k_cut, dict(mode="first_above_quantile", reason="no cand; used argmax", thr=thr, smoothed=sm, diffs=d, score=score, i_star=i_star, k_cut=k_cut)

        i_star = int(cand[0])
        k_cut = int(k_list[i_star])
        return k_cut, dict(mode="first_above_quantile", thr=thr, smoothed=sm, diffs=d, score=score, i_star=i_star, k_cut=k_cut)

    else:
        raise ValueError("changepoint_mode must be 'largest_jump' or 'first_above_quantile'.")
...
def eps_from_signal_quantile(
    ...
    *,
    ...
    info = dict(p_op_used=float(p_list[j]), q=float(q), pool_desc=pool_desc, threshold=thr, eps=eps, use_lower_ci=bool(use_lower_ci))
    return eps, info
...
def _find_first_run_ge_threshold(p_list: np.ndarray, curve: np.ndarray, thr: float, run: int = 1) -> Optional[float]:
    ...
    *,
    ...
    if mode == "kappa_changepoint":
        ...
        if _BETA_CDF is None:
            raise ValueError("beta_null mode requires SciPy (stats.beta.cdf or special.betainc).")
        n = int(n_dim)
        ...
        raise ValueError("K_sig_mode must be one of: 'kappa_changepoint', 'beta_null', 'none'.")
    ...
    recommended_global = p_star_reference if p_star_reference is not None else p_star_global_signals
    ...
    if make_plots:
        ...
        ax_curves.plot(p_list, noise_ref_curve, linestyle="--", linewidth=2, label=f"noise q={float(noise_quantile):.2f}")
        ax_curves.plot(p_list, signal_ref_curve, linewidth=2.5, label=signal_ref_label)
        ax_curves.axhline(0.0, alpha=0.0)
        ax_curves.set_title("Selected $I^{\\mathrm{proj}}_k(p)$ curves with signal/noise references")
        ...
        ax_kappa.plot(k_list, kappa_hat, marker=".", linestyle="-", label="$\\hat\\kappa_k$")
        if mode == "kappa_changepoint" and "k_cut" in K_sig_details:
            k_cut = int(K_sig_details["k_cut"])
            ax_kappa.axvline(k_cut + 0.5, linestyle="--", linewidth=1.5, label=f"cut at k={k_cut}")
        if len(K_sig_indices) > 0:
            ax_kappa.scatter(k_list[K_sig_mask], kappa_hat[K_sig_mask], s=60, marker="s", label=r"$k\in\mathcal{K}_{\mathrm{sig}}$")
        ...
        ax_lift.plot(p_list, noise_ref_curve, linestyle="--", linewidth=2, label=f"noise q={float(noise_quantile):.2f}")
        ax_lift.plot(p_list, ref_gap_curve, linestyle=":", linewidth=2, label="signal - noise")
        ax_lift.axhline(float(lift_margin), color="k", linestyle="-.", linewidth=1.2, label=f"margin={float(lift_margin):.3f}")
        if p_star_reference is not None:
            ax_lift.axvline(float(p_star_reference), color="k", linestyle="--", linewidth=1.2, label=f"reference p*={float(p_star_reference):.3f}")
        ...
        if len(K_sig_indices) > 0:
            ax_ps.scatter(k_list[K_sig_mask], vals[K_sig_mask], s=60, marker="s", label=r"$k\in\mathcal{K}_{\mathrm{sig}}$")
        if p_star_reference is not None:
            ax_ps.axhline(float(p_star_reference), linestyle="--", linewidth=1.2, label=f"reference p*={float(p_star_reference):.3f}")
...
def analyze_iproj_signal_alignment_v4(
    Iproj_boot: np.ndarray,
    lambda_topk_boot: np.ndarray,
    k_list: Union[List[int], np.ndarray],
    p_list: Union[List[float], np.ndarray],
    *,
    ci: float = 0.90,
    ...
    p_cap: float = 0.90,
    fdr_q: float = 0.10,
    eig_curve_mode: str = "lower_ci",      # "lower_ci" or "median"
    bulk_edge_curve: Optional[np.ndarray] = None,
    bulk_quantile: float = 0.90,
    edge_margin: float = 0.0,
    require_consecutive: int = 1,
    target_overlap: float = 0.90,
    leakage_p_mode: str = "grid",          # "grid" or "continuous"
    signal_reference: str = "boundary",    # "boundary" or "signal_quantile"
    signal_quantile: float = 0.10,
    make_plots: bool = True,
    max_curve_plots: int = 8,
    ...
    """
    Hybrid v4 analysis combining eigenspace coherence with spectral separation.

    Compared with ``analyze_iproj_signal_alignment_v3``, this routine keeps the
    same data-driven ``k^*`` detection logic but replaces the final ``p^*`` rule
    with a hybrid criterion:

    1. ``p_edge``: the smallest masking level where the observed ``k``-th eigenvalue
       clears an empirical bulk-edge curve built from the noise-index eigenvalue pool.
    2. ``p_leak``: the smallest masking level predicted by the leakage model
          1 - I_k^{proj}(p) ~ kappa_k (1-p)/p
       needed to achieve a target overlap ``target_overlap``.

    The per-component threshold is then
        p_k^* = max(p_edge,k, p_leak,k),
    and the global recommendation is the maximum over the detected signal set.
    """
    Iproj_boot = np.asarray(Iproj_boot, dtype=float)
    lambda_topk_boot = np.asarray(lambda_topk_boot, dtype=float)
    k_list = np.asarray(k_list, dtype=int)
    p_list = np.asarray(p_list, dtype=float)

    if Iproj_boot.ndim != 3:
        raise ValueError("Iproj_boot must have shape (K_count, P_count, B).")
    if lambda_topk_boot.ndim != 3:
        raise ValueError("lambda_topk_boot must have shape (Kmax, P_count, B).")

    K_count, P_count, B = Iproj_boot.shape
    Kmax, P_lam, B_lam = lambda_topk_boot.shape
    if P_lam != P_count or B_lam != B:
        raise ValueError("lambda_topk_boot must align with Iproj_boot along p and bootstrap axes.")
    if k_list.size != K_count:
        raise ValueError("k_list length must match Iproj_boot.shape[0].")
    if p_list.size != P_count:
        raise ValueError("p_list length must match Iproj_boot.shape[1].")
    if np.any(np.diff(p_list) < 0):
        raise ValueError("p_list must be sorted increasing.")
    if np.any(k_list <= 0) or np.any(k_list > Kmax):
        raise ValueError("All entries of k_list must lie in {1, ..., lambda_topk_boot.shape[0]}." )
    if not (0.0 < float(target_overlap) < 1.0):
    ...
    k_idx = k_list - 1
    lambda_sel_boot = lambda_topk_boot[k_idx, :, :]
    lambda_med = np.nanmedian(lambda_sel_boot, axis=2)
    lambda_lo = np.nanquantile(lambda_sel_boot, q_lo, axis=2)
    lambda_hi = np.nanquantile(lambda_sel_boot, q_hi, axis=2)

    if eig_curve_mode == "lower_ci":
        signal_lambda_curves = lambda_lo
    elif eig_curve_mode == "median":
        signal_lambda_curves = lambda_med
    else:
        raise ValueError("eig_curve_mode must be 'lower_ci' or 'median'.")

    noise_pool_desc = "supplied_bulk_edge_curve"
    if bulk_edge_curve is not None:
        bulk_edge_curve = np.asarray(bulk_edge_curve, dtype=float)
        if bulk_edge_curve.shape != (P_count,):
            raise ValueError("bulk_edge_curve must have shape (len(p_list),).")
    else:
        noise_pool_desc = "noise_only"
        if len(K_noise_indices) > 0:
            bulk_pool = lambda_sel_boot[K_noise_mask, :, :]
        else:
            start = max(K_count // 2, 1)
            bulk_pool = lambda_sel_boot[start:, :, :]
            noise_pool_desc = "fallback_upper_half_k"
        bulk_edge_curve = np.nanquantile(
            np.transpose(bulk_pool, (1, 0, 2)).reshape(P_count, -1),
            float(bulk_quantile),
            axis=1,
        )

    target_leakage = float(np.clip(1.0 - float(target_overlap), 1e-6, 1.0 - 1e-6))
    p_leak_cont_by_k = np.clip(kappa_hat / (kappa_hat + target_leakage), 0.0, 1.0)
    p_leak_grid_by_k: List[Optional[float]] = []
    for val in p_leak_cont_by_k:
        idx = np.where(p_list >= float(val))[0]
        p_leak_grid_by_k.append(None if idx.size == 0 else float(p_list[int(idx[0])]))

    p_edge_by_k: List[Optional[float]] = []
    edge_gap_by_k = []
    for i in range(K_count):
        gap = signal_lambda_curves[i] - bulk_edge_curve
        edge_gap_by_k.append(gap)
        p_edge_by_k.append(
            _find_first_run_ge_threshold(
                p_list,
                gap,
    ...
    edge_gap_by_k = np.asarray(edge_gap_by_k, dtype=float)

    p_star_by_k: List[Optional[float]] = []
    p_star_partial_by_k: List[Optional[float]] = []
    for i in range(K_count):
        p_edge = p_edge_by_k[i]
        p_leak = p_leak_cont_by_k[i] if leakage_p_mode == "continuous" else p_leak_grid_by_k[i]
        if p_edge is None or p_leak is None:
            p_star_by_k.append(None)
        else:
            p_star_by_k.append(float(max(p_edge, p_leak)))

        avail = [v for v in (p_edge, p_leak) if v is not None]
        p_star_partial_by_k.append(None if len(avail) == 0 else float(max(avail)))
    ...
    if signal_reference == "boundary":
        ...
        if len(K_sig_indices) == 0:
            ...
            pstar_sig = np.array([
                np.nan if p_star_by_k[i] is None else float(p_star_by_k[i])
                for i in K_sig_indices
            ], dtype=float)
            if np.all(np.isnan(pstar_sig)):
                ref_idx = int(K_sig_indices[-1])
                ref_info["fallback"] = "boundary_signal"
                ref_info.update(ref_k=int(k_list[ref_idx]), ref_index=ref_idx)
            else:
                qv = float(np.nanquantile(pstar_sig, float(signal_quantile)))
                ref_idx = int(K_sig_indices[np.nanargmin(np.abs(pstar_sig - qv))])
                ref_info.update(ref_k=int(k_list[ref_idx]), ref_index=ref_idx, signal_quantile=float(signal_quantile))
    else:
        raise ValueError("signal_reference must be 'boundary' or 'signal_quantile'.")

    p_edge_reference = p_edge_by_k[ref_idx]
    p_leak_reference = p_leak_cont_by_k[ref_idx] if leakage_p_mode == "continuous" else p_leak_grid_by_k[ref_idx]
    p_star_reference = None if (p_edge_reference is None or p_leak_reference is None) else float(max(p_edge_reference, p_leak_reference))
    p_star_reference_partial = None
    _ref_avail = [v for v in (p_edge_reference, p_leak_reference) if v is not None]
    if len(_ref_avail) > 0:
        p_star_reference_partial = float(max(_ref_avail))

    finite_all = np.array([np.nan if v is None else float(v) for v in p_star_by_k], dtype=float)
    finite_partial_all = np.array([np.nan if v is None else float(v) for v in p_star_partial_by_k], dtype=float)

    p_star_global_all = None
    if np.any(np.isfinite(finite_all)):
        p_star_global_all = float(np.nanmax(finite_all))

    p_star_global_signals = None
    p_star_global_signals_partial = None
    if len(K_sig_indices) > 0:
        vals = np.array([np.nan if p_star_by_k[i] is None else float(p_star_by_k[i]) for i in K_sig_indices], dtype=float)
        vals_partial = np.array([np.nan if p_star_partial_by_k[i] is None else float(p_star_partial_by_k[i]) for i in K_sig_indices], dtype=float)
        if np.any(np.isfinite(vals)):
            p_star_global_signals = float(np.nanmax(vals))
        if np.any(np.isfinite(vals_partial)):
            p_star_global_signals_partial = float(np.nanmax(vals_partial))

    recommended_global = None
    recommended_global_source = None
    if p_star_reference is not None:
        recommended_global = float(p_star_reference)
        recommended_global_source = "reference_strict"
    elif p_star_global_signals is not None:
        recommended_global = float(p_star_global_signals)
        recommended_global_source = "signals_strict_max"
    elif p_star_reference_partial is not None:
        recommended_global = float(p_star_reference_partial)
        recommended_global_source = "reference_partial"
    elif p_star_global_signals_partial is not None:
        recommended_global = float(p_star_global_signals_partial)
        recommended_global_source = "signals_partial_max"
    elif np.any(np.isfinite(finite_partial_all)):
        recommended_global = float(np.nanmax(finite_partial_all))
        recommended_global_source = "all_partial_max"

    summary = dict(
        I_median=I_med,
        I_ci_low=I_lo,
        I_ci_high=I_hi,
        lambda_median=lambda_med,
        lambda_ci_low=lambda_lo,
        lambda_ci_high=lambda_hi,
        ci_level=float(ci),
        kappa_hat=kappa_hat,
        kappa_info=kappa_info,
        K_sig_mode=K_sig_mode,
        ...
        K_sig_k_values=K_sig_k_values,
        K_noise_k_values=K_noise_k_values,
        p_star_definition="hybrid_edge_plus_leakage",
        eig_curve_mode=str(eig_curve_mode),
        bulk_quantile=float(bulk_quantile),
        bulk_edge_curve=np.asarray(bulk_edge_curve, dtype=float),
        edge_gap_by_k=edge_gap_by_k,
        edge_margin=float(edge_margin),
        require_consecutive=int(require_consecutive),
        target_overlap=float(target_overlap),
        target_leakage=float(target_leakage),
        leakage_p_mode=str(leakage_p_mode),
        noise_pool_desc=noise_pool_desc,
        reference_info=ref_info,
        recommended_global_source=recommended_global_source,
    )

    p_star = dict(
        by_k=p_star_by_k,
        partial_by_k=p_star_partial_by_k,
        p_edge_by_k=p_edge_by_k,
        p_leak_cont_by_k=p_leak_cont_by_k.tolist(),
        p_leak_grid_by_k=p_leak_grid_by_k,
        global_all=p_star_global_all,
        global_signals=p_star_global_signals,
        global_signals_partial=p_star_global_signals_partial,
        reference_signal=p_star_reference,
        reference_signal_partial=p_star_reference_partial,
        reference_edge=p_edge_reference,
        reference_leak=p_leak_reference,
        recommended_global=recommended_global,
        recommended_global_source=recommended_global_source,
        K_sig_k_values=K_sig_k_values,
    )
    ...
    if make_plots:
        ...
        fig_curves, ax_curves = plt.subplots(figsize=figsize_curves)
        for i in sel_idx:
            ax_curves.plot(p_list, signal_lambda_curves[i], label=f"k={k_list[i]}")
            ax_curves.fill_between(p_list, lambda_lo[i], lambda_hi[i], alpha=0.12)
        ax_curves.plot(p_list, bulk_edge_curve, linestyle="--", linewidth=2, label=f"bulk edge q={float(bulk_quantile):.2f}")
        if p_star_reference is not None:
            ax_curves.axvline(float(p_star_reference), color="k", linestyle="--", linewidth=1.2, label=f"reference p*={float(p_star_reference):.3f}")
        ax_curves.set_title("Selected eigenvalue curves against empirical bulk edge")
        ax_curves.set_xlabel("$p$")
        ax_curves.set_ylabel("eigenvalue / bulk edge")
        ax_curves.legend(ncols=2, fontsize=9)
        figures["lambda_selected"] = fig_curves
        ...
        fig_edge, ax_edge = plt.subplots(figsize=figsize_liftoff)
        ax_edge.plot(p_list, signal_lambda_curves[ref_idx], linewidth=2.5, label=f"signal ref k={int(k_list[ref_idx])}")
        ax_edge.plot(p_list, bulk_edge_curve, linestyle="--", linewidth=2, label=f"bulk edge q={float(bulk_quantile):.2f}")
        ax_edge.plot(p_list, edge_gap_by_k[ref_idx], linestyle=":", linewidth=2, label="signal - bulk edge")
        ax_edge.axhline(float(edge_margin), color="k", linestyle="-.", linewidth=1.2, label=f"edge margin={float(edge_margin):.3f}")
        if p_edge_reference is not None:
            ax_edge.axvline(float(p_edge_reference), color="tab:blue", linestyle="--", linewidth=1.2, label=f"p_edge={float(p_edge_reference):.3f}")
        if p_leak_reference is not None:
            ax_edge.axvline(float(p_leak_reference), color="tab:orange", linestyle="--", linewidth=1.2, label=f"p_leak={float(p_leak_reference):.3f}")
        if p_star_reference is not None:
            ax_edge.axvline(float(p_star_reference), color="k", linestyle="-", linewidth=1.4, label=f"p*={float(p_star_reference):.3f}")
        ax_edge.set_title("Reference spectral emergence and hybrid threshold")
        ax_edge.set_xlabel("$p$")
        ax_edge.set_ylabel("curve / gap value")
        ax_edge.legend(fontsize=9)
        figures["hybrid_reference"] = fig_edge

        fig_ps, ax_ps = plt.subplots(figsize=figsize_curves)
        vals = np.array([np.nan if v is None else v for v in p_star_by_k], dtype=float)
        vals_edge = np.array([np.nan if v is None else v for v in p_edge_by_k], dtype=float)
        vals_leak = np.array([np.nan if v is None else v for v in p_leak_grid_by_k], dtype=float)
        ax_ps.plot(k_list, vals_edge, marker="o", linestyle="--", label="$p_{k,\mathrm{edge}}$")
        ax_ps.plot(k_list, vals_leak, marker="^", linestyle=":", label="$p_{k,\mathrm{leak}}$")
        ax_ps.plot(k_list, vals, marker="s", linestyle="-", label="$p_k^*$")
        if len(K_sig_indices) > 0:
            ax_ps.scatter(k_list[K_sig_mask], vals[K_sig_mask], s=60, marker="s", label=r"$k\in\mathcal{K}_{\mathrm{sig}}$")
        if p_star_reference is not None:
            ax_ps.axhline(float(p_star_reference), linestyle="--", linewidth=1.2, label=f"reference p*={float(p_star_reference):.3f}")
        ax_ps.set_title("Hybrid $p_k^*$ by $k$")
        ax_ps.set_xlabel("$k$")
        ax_ps.set_ylabel("threshold in $p$")
        ax_ps.set_ylim(0, 1.02)
        ax_ps.legend(fontsize=9)
        figures["pstar_vs_k"] = fig_ps
