"""CLIP ViT-L/14 (openai) on THINGS+ 1854 — RBF kernel + p* + coarse CV.

Source: pre-extracted features in deep-similarity:
    /SSD/projects/deep-similarity/data/features/things22k/clip_vit_l_14.npy   (22248, 1024)
    /SSD/projects/deep-similarity/data/meta/things22k/image_info_22k.csv

Per-category "plus" image is the _01[bsn].jpg row (exactly 1854 total).
NO averaging — one image per category, matching the vgg16 convention.

Pipeline:
  1. Load + filter to 1854 plus rows, alpha-sort by category.
  2. RBF kernel with median-heuristic sigma.
  3. estimate_rank -> k_cut, p*.
  4. cross_val_score on coarse grid [50, 100, 150, 200] + k_cut.

Run:
    poetry run python sandbox/dimensionality/clip_rbf_cv/run.py
"""
...
FEAT_PATH = Path("/data/labshare/_stachelschwein/SSD/projects/deep-similarity/data/features/things22k/clip_vit_l_14.npy")
META_PATH = Path("/data/labshare/_stachelschwein/SSD/projects/deep-similarity/data/meta/things22k/image_info_22k.csv")
...
def select_plus_rows(meta: pd.DataFrame) -> pd.DataFrame:
    """One row per category — the _01[bsn].jpg image (the THINGS+ representative)."""
    mask = meta["filename"].str.match(r".+_01[bsn]\.jpg$")
    plus = meta.loc[mask].copy()
    # validate: exactly one per category, all 1854
    assert plus["category"].is_unique, "multiple _01 entries per category"
    assert len(plus) == 1854, f"expected 1854 plus rows, got {len(plus)}"
    plus = plus.sort_values("category").reset_index(drop=False)
    plus = plus.rename(columns={"index": "orig_row"})
    return plus


def main() -> None:
    log(f"loading features from {FEAT_PATH}")
    x_22k = np.load(FEAT_PATH)
    log(f"  features shape={x_22k.shape}  dtype={x_22k.dtype}")

    log(f"loading metadata from {META_PATH}")
    meta = pd.read_csv(META_PATH)
    plus = select_plus_rows(meta)
    log(f"  selected plus rows: {len(plus)}  (1854 unique categories)")
    plus.to_csv(OUT / "plus_metadata.csv", index=False)

    rows = plus["orig_row"].to_numpy()
    x = x_22k[rows].astype(np.float64)
    log(f"  filtered features shape={x.shape}  (one _01 image per category, alpha-sorted)")
    np.save(OUT / "clip_vit_l14_features_1854.npy", x)

    log("building RBF (gaussian) kernel similarity with median-heuristic sigma ...")
    t0 = time.time()
    s = gaussian_kernel_similarity(x, x).astype(np.float64)
    log(f"  built in {time.time()-t0:.1f}s  S shape={s.shape}  "
        f"range=[{s.min():.4f}, {s.max():.4f}]  median_off={np.median(s[np.triu_indices(s.shape[0],1)]):.4f}")
    np.save(OUT / "clip_rbf.npy", s)
    ...
    ax.set_title(f"CLIP ViT-L/14 RBF  n=1854  p*={est.sampling_fraction:.3f}  argmin={argmin_rank}")
