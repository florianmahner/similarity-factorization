"""CLIP RN50 on THINGS+ 1854 — COSINE kernel + p* + coarse CV.

Cosine = X_normalized @ X_normalized.T (row-normalize then linear kernel).
Removes magnitude variation. Same spectrum-shape question as linear, but
without the heavy outer-product magnitudes.

Run:
    poetry run python sandbox/dimensionality/clip_rn50_cosine/run.py
"""
...
def main() -> None:
    ...
    log("building COSINE kernel similarity ...")
    t0 = time.time()
    s = cosine_similarity(x, x).astype(np.float64)
    # cosine returns values in [-1, 1]; diagonal should be 1 (within fp tolerance)
    off = s[np.triu_indices(s.shape[0], k=1)]
    log(f"  built in {time.time()-t0:.1f}s  shape={s.shape}  "
        f"diag mean={np.diag(s).mean():.6f}  off range=[{off.min():.4f}, {off.max():.4f}]  "
        f"median_off={np.median(off):.4f}  frac_negative={(off < 0).mean():.4f}")
    np.save(OUT / "s_cosine.npy", s)
    ...
    for _, row in summary.iterrows():
        ...
        if int(row["rank"]) == int(est.rank) and int(row["rank"]) != argmin_rank:
            marker += "  <- k_cut"
        log(f"{int(row['rank']):>5}  {row['mean']:>12.6e}  {row['sem']:>10.4e}{marker}")

    fig, ax = plt.subplots(figsize=(5.8, 3.6))
    ax.errorbar(summary["rank"], summary["mean"], yerr=summary["sem"],
                marker="o", ms=5, lw=1.6, color="C3")
    ax.scatter([argmin_rank],
               [float(summary.loc[summary["rank"] == argmin_rank, "mean"].iloc[0])],
               marker="*", s=180, color="C3", edgecolors="white", linewidths=1.2, zorder=10)
    ax.axvline(est.rank, color="gray", ls="--", lw=0.9, label=f"k_cut={est.rank}")
    ax.set_xlabel("rank")
    ax.set_ylabel("validation MSE")
    ax.set_title(f"CLIP RN50 COSINE  n=1854  p*={est.sampling_fraction:.3f}  argmin={argmin_rank}")
