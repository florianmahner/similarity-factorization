...
def mixed_sign_nmf_cd_with_net_nonneg_residual(
    ...
    s_plus, s_minus = split_matrix(s)

    for it in range(n_iter):
        # positive block
        zp = s_plus + w_minus @ h_minus.T
        w_plus = update_block(
        ...
        # – block with elementwise bounds, added this based on mixing!
        Zn = w_plus @ h_plus.T - s_minus
...
def split_matrix(s):
    ...
    s_plus = copy.deepcopy(s)
    s_plus = s_plus - threshold
    s_plus[s_plus < 0] = 0
    s_minus = copy.deepcopy(s)
...
def make_positive_and_negative_matrices(n, r, random_state=42):
    ...
    s = w_p @ h_p.T - w_n @ h_n.T
    return s, w_p, h_p, w_n, h_n
...
# %%
n, r = 200, 5
s, wp_true, hp_true, wn_true, hn_true = make_positive_and_negative_matrices(n, r)

wp, hp, wn, hn, history = mixed_sign_nmf_cd_with_net_nonneg(
    s, k=r, n_iter=300, alpha=1.0, random_state=42
)


print("shapes:", wp.shape, hp.shape, wn.shape, hn.shape)
assert np.all(wp - wn >= -1e-8)
assert np.all(hp - hn >= -1e-8)
print("success: net factors are elementwise nonnegative!")

print("Evar(wp, wp_true):", evar(wp_true, wp))
print("Evar(hp, hp_true):", evar(hp_true, hp))
print("Evar(wn, wn_true):", evar(wn_true, wn))
print("Evar(hn, hn_true):", evar(hn_true, hn))

# %%

wp, hp, wm, hm, history = mixed_sign_nmf_cd_with_net_nonneg_residual(
    S, k=66, n_iter=500, alpha=1.0, random_state=42
)
